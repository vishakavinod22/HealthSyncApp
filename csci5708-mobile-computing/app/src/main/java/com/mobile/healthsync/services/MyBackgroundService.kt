...
class MyBackgroundService : Service() {
    ...
    private fun sendNotifications() {
        apiService.sendNotifications().enqueue(object : Callback<ApiResponse> {
            override fun onResponse(call: Call<ApiResponse>, response: Response<ApiResponse>) {
                if (response.isSuccessful) {
                    val apiResponse = response.body()
                    if (apiResponse != null && apiResponse.status == "success") {
                        Log.d("MyBackgroundService", "Notification sent successfully")
                    } else {
                        Log.e("MyBackgroundService", "Failed to send notification. Response message: ${apiResponse?.message ?: "Unknown error"}")
                    }
                } else {
                    Log.e("MyBackgroundService", "Failed to send notification. Response code: ${response.code()}")
                }
